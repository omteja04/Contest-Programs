/**
 * Author: omteja04
 * Created on: 28-12-2024 00:17:43
 * Description: B_The_Meeting_Place_Cannot_Be_Changed
 **/
...
bool can(int n, vi &pos, vi &m, ld time) {
    ld miniR = 1e9 + time;
    ld maxiL = 1 - time;
    fn(i, n) {
        miniR = std::min(miniR, pos[i] + time * m[i]);
        maxiL = std::max(maxiL, pos[i] - time * m[i]);
    }
    return maxiL <= miniR;
}
void levi() {
    int noOfFriends;
    std::cin >> noOfFriends;
    vi pos(noOfFriends), meters(noOfFriends);
    INPUT_ARRAY_1D(noOfFriends, pos);
    INPUT_ARRAY_1D(noOfFriends, meters);
    ld low = 0;
    ld high = 1e9;
    ld PRECISION = 1e-7;
    ld ans = 0;
    while(low <= high) {
        ...
        if(can(noOfFriends, pos, meters, mid)) {
            ans = mid;
            high = mid - PRECISION;
        } else {
            low = mid + PRECISION;
