/**
 * Author: omteja04
 * Created on: 31-05-2025 20:28:05
...
void levi() {
    ...
    vector<int> canUseFrom(n);
    for(int i = 0; i < n; i++) {
        canUseFrom[i] = min(pos_p[i], pos_q[i]);
    }
    int max_pow = n + 10;
    vector<long long> pow2(max_pow, 1);
    for(int i = 1; i < max_pow; i++) {
        pow2[i] = (pow2[i - 1] * 2) % MOD;
    }
    vector<vector<int>> useAt(n);
    for(int v = 0; v < n; v++) {
        useAt[canUseFrom[v]].push_back(v);
    }
