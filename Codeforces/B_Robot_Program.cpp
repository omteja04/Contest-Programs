/**
 * Author: omteja04
 * Created on: 27-02-2025 20:33:48
...
void levi() {
    ...
    movements[0] = (str[0] == 'L') ? -1 : 1;
    bool minFound = false;
    int movementInd = INT_MAX;
    for(int i = 1; i < n; i++) {
        movements[i] = movements[i - 1] + (str[i] == 'L' ? -1 : 1);
        if(movements[i] == 0 && !minFound) {
            minFound = true;
            movementInd = i + 1;
        }
    }
    ...
    if(k == 0 || !minFound || !zeros) {
        cout << zeros;
        return;
    }
    zeros = k / movementInd + (k % movementInd != 0);
    cout << zeros;
}
#undef int
