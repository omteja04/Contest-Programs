/**
 * Author: omteja04
 * Created on: 22-03-2025 21:25:44
 * Problem: C_Serval_and_The_Formula
 * Link: https://codeforces.com/contest/2085/problem/C
 **/
...
void levi() {
    ...
    bool dp[MAX_BIT + 1][2][2];
    memset(dp, false, sizeof(dp));

    tuple<int, int, int> par[MAX_BIT + 1][2][2];
    dp[0][0][0] = true;
        ...
        for(int cx = 0; cx < 2; cx++) {
            for(int cy = 0; cy < 2; cy++) {
                ...
                for(int k_bit = 0; k_bit < 2; k_bit++) {
                    int s1 = a + k_bit + cx, newBit1 = s1 & 1, newCX = s1 >> 1;
                    int s2 = b + k_bit + cy, newBit2 = s2 & 1, newCY = s2 >> 1;
                    if(newBit1 == 1 && newBit2 == 1) {
                        continue;
                    }
                    if(!dp[i + 1][newCX][newCY]) {
                        dp[i + 1][newCX][newCY] = true;
                        par[i + 1][newCX][newCY] = {cx, cy, k_bit};
                    }
                }
            }
        }
    ...
    for(int i = MAX_BIT; i > 0; i--) {
        int prev_cx, prev_cy, chosen;
        tie(prev_cx, prev_cy, chosen) = par[i][cur_cx][cur_cy];
        if(chosen == 1) {
            k |= (1LL << (i - 1));
        }
        cur_cx = prev_cx;
