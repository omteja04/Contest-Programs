/**
 * Author: omteja04
 * Created on: 06-01-2025 13:39:44
 * Description: B_Travel_Card
 **/
...
void levi() {
    int trips;
    std::cin >> trips;
    std::vector<int> minutes(trips);
    for(int &journey: minutes) {
        std::cin >> journey;
    }
    std::vector<int> dp(trips, 0);
    for(int i = 0; i < trips; i++) {
        int _90minBack = std::upper_bound(minutes.begin(), minutes.begin() + i, minutes[i] - 90) - minutes.begin();
        int _1440minBack = std::upper_bound(minutes.begin(), minutes.begin() + i, minutes[i] - 1440) - minutes.begin();
        if(i != 0) {
            dp[i] = dp[i - 1] + 20;
        } else {
            dp[i] = 20;
        ...
        if(_90minBack != 0) {
            dp[i] = std::min(dp[i], dp[_90minBack - 1] + 50);
        } else {
            dp[i] = std::min(dp[i], 50 * 1ll);
        }
        if(_1440minBack != 0) {
            dp[i] = std::min(dp[i], dp[_1440minBack - 1] + 120);
        } else {
            dp[i] = std::min(dp[i], 120 * 1ll);
        }
    }
    for(int i = 0; i < trips; i++) {
        if(i == 0) {
