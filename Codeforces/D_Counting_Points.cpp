/**
 * Author: omteja04
 * Created on: 11-03-2025 21:23:27
 * Problem: D_Counting_Points
...
void levi() {
    int n, m;
    cin >> n >> m;
    unordered_map<int, int> maxYCanBeReached;
    vector<int> centersX(n), radii(n);
    INPUT_ARRAY_1D(n, centersX);
    INPUT_ARRAY_1D(n, radii);
    for(int i = 0; i < n; i++) {
        int cx = centersX[i];
        int radius = radii[i];
        for(int x = cx - radius; x <= cx + radius; x++) {
            int y = floor(sqrt(radius * radius - (x - cx) * (x - cx)));
            maxYCanBeReached[x] = max(maxYCanBeReached[x], y);
        }
    }
    int integerPoints = 0;
    for(auto &it: maxYCanBeReached) {
        integerPoints = integerPoints + 2 * it.second + 1;
    }
    cout << integerPoints;
}
#undef int
