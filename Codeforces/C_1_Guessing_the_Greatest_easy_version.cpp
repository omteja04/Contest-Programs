/**
 * Author: omteja04
 * Created on: 06-01-2025 15:47:28
 * Description: C_1_Guessing_the_Greatest_easy_version
 **/
...
void levi() {
    ...
    while(high - low > 1) {
        int mid = middle;
        int _2ndMax = ask(low, high - 1);
        if(_2ndMax < mid) {
            if(ask(low, mid - 1) == _2ndMax) {
                high = mid;
            } else {
                low = mid;
            }
        } else {
            if(ask(mid, high - 1) == _2ndMax) {
                low = mid;
            } else {
                high = mid;
            }
