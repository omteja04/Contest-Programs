/**
 * Author: omteja04
 * Created on: 27-02-2025 21:48:26
 * Problem: C_Limited_Repainting
...
=================================================
*/
bool canPaint(int mid, vector<int> &penalty, vector<char> &req, int k) {
    int n = req.size();
    int operations = 0;
    for(int i = 0; i < n;) {
        if(req[i] == 'B' && penalty[i] > mid) {
            operations++;
            while(i < n && (req[i] == 'B' || penalty[i] <= mid)) {
                i++;
            }
        } else {
            i++;
...
ll minimumPenalty(vector<char> &req, vector<int> &penalty, int n, int k) {
    int low = 0;
    int high = *max_element(all(penalty));
    int ans = high;
    while(low <= high) {
        int mid = low + (high - low) / 2;
        if(canPaint(mid, penalty, req, k)) {
            ans = mid;
            high = mid - 1;
        } else {
            low = mid + 1;
...
void levi() {
    ...
    cout << minimumPenalty(req, penalty, n, k);
}
#undef int
