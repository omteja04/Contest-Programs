/**
 * Author: omteja04
 * Created on: 28-05-2025 20:54:23
 * Problem: Swaps_in_a_String
 * Link: https://www.codechef.com/START188B/problems/SWAPSTR
 **/
#include <bits/stdc++.h>
#include <vector>
using namespace std;
...
void levi() {
    ...
    vector<int> suffixC(n + 2, 0);

    for(int i = 0; i < n; i++) {
        prefixA[i + 1] = prefixA[i] + (s[i] == 'A' ? 1 : 0);
    }
    for(int i = n - 1; i >= 0; i--) {
        suffixC[i] = suffixC[i + 1] + (s[i] == 'C' ? 1 : 0);
    }
    vector<int> lastCIdx(n, -1);
    int lastC = -1;
    for(int i = 0; i < n; i++) {
        if(s[i] == 'C') {
            lastC = i;
    ...
    for(int i = 0; i < n; i++) {
        if(s[i] == 'B') {
            ...
            int leftBound = lastCIdx[i];
            long long swappableA = (leftBound == -1) ? prefixA[i] : prefixA[i] - prefixA[leftBound + 1];

            int rightBound = nextAIdx[i];
            long long swappableC = (rightBound == n) ? suffixC[i + 1] : suffixC[i + 1] - suffixC[rightBound];

            ans += max(swappableA, swappableC);
        }
    }
