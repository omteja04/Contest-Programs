/**
 * Author: omteja04
 * Created on: 01-01-2025 20:05:27
 * Description: Lottery_Tickets
 **/
...
void levi() {
    ...
    int ticket = arr[0];
    std::sort(all(arr));
    auto lowerIT = std::lower_bound(all(arr), ticket);
    auto upperIT = std::upper_bound(all(arr), ticket);
    int lower, upper, prevLower;
    if(arr[0] == ticket) {
        std::cout << ticket + (arr[1] - ticket) / 2;
        return;
    } else if(arr[n - 1] == ticket) {
        std::cout << 1000000 - ticket + 1 + (ticket - arr[n - 2]) / 2;
        return;
    } else {
        int a = 0;
        int b = 0;
        for(int i = 0; i < n; i++) {
            if(arr[i] == ticket) {
                ...
                a = ticket - (ticket - arr[i - 1]) / 2;
                b = ticket + (arr[i + 1] - ticket) / 2;
                debug(a);
                debug(b);
                break;
            }
