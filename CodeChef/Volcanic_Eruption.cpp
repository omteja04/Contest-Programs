/**
 * Author: omteja04
 * Created on: 29-01-2025 21:35:01
 * Description: Volcanic_Eruption
 **/
...
void levi() {
    ...
    for(int i = 0; i < n; i++) {
        if(arr[i] == 0) {
            res[i] = 0;
            int ans = 0;
            for(int left = i - 1; left >= 0 && arr[left] != 0; left--) {
                int val = arr[left] / k;
                if(arr[left] % k != 0) {
                    val++;
                }
                ans = std::max(val, ans);
                res[left] = std::min(res[left], ans);
            }
            ans = 0;
            for(int right = i + 1; right < n && arr[right] != 0; right++) {
                int val = arr[right] / k;
                if(arr[right] % k != 0) {
                    val++;
                }
                ans = std::max(val, ans);
                res[right] = std::min(res[right], ans);
            }
        }
    }
    for(int i = 0; i < n; i++) {
