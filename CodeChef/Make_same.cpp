/**
 * Author: omteja04
 * Created on: 26-02-2025 20:12:44
 * Problem: Make_same
 * Link: https://www.codechef.com/START175C/problems/P3_175
 **/
#include <bits/stdc++.h>
#include <cmath>
#include <vector>
...
void levi() {
    ...
    int s11 = 0, s10 = 0, s21 = 0, s20 = 0, s30 = 0, s31 = 0;
    for(int i = 0; i < n; i++) {
        if(str[0][i] == '1') {
            s11++;
        } else {
    ...
    if(s10 % n == 0 && s20 % n == 0 && s30 % n == 0) {
        cout << 0;
        return;
    }
    int needed0 = min({s10, s20, s30});
    int needed1 = min({s21, s31, s11});
    cout << max(needed0, needed1);
}
#undef int
