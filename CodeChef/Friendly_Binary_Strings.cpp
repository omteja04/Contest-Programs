/**
 * Author: omteja04
 * Created on: 05-03-2025 21:51:59
 * Problem: Friendly_Binary_Strings
 * Link: https://www.codechef.com/START176B/problems/MSATP
 **/
...
void levi() {
    ...
    if(n % 2 == 0) {
        if(mixedCnt % 2 == 0 && onesCnt % 2 == 0 && zerosCnt % 2 == 0) {
            yes();
        } else {
            no();
        }
    } else {
        if((mixedCnt % 2 + onesCnt % 2 + zerosCnt % 2) == 1) {
            yes();
        } else {
            no();
        }
