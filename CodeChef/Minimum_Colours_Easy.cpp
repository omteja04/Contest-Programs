/**
 * Author: omteja04
 * Created on: 19-03-2025 20:31:39
 * Problem: Minimum_Colours_Easy
 * Link: https://www.codechef.com/START178B/problems/MINCOL
 **/
#include <algorithm>
#include <bits/stdc++.h>
#include <climits>
...
void levi() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        if(mp.count(arr[i - 1])) {
            dp[i] = min(dp[i], 1 + mp[arr[i - 1]]);
        }

        if(!mp.count(arr[i - 1])) {
            mp[arr[i - 1]] = dp[i - 1];
        } else {
            mp[arr[i - 1]] = min(mp[arr[i - 1]], dp[i - 1]);
        }
    }
