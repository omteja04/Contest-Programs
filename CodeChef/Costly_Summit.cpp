/**
 * Author: omteja04
 * Created on: 05-03-2025 20:19:20
 * Problem: Costly_Summit
 * Link: https://www.codechef.com/START176B/problems/LSU
 **/
#include <algorithm>
#include <bits/stdc++.h>
#include <vector>
...
void levi() {
    ...
    int translatorForAll = findSum(n);

    int cost = c * mp.size();

    int miniCost = min(translatorForAll, cost);
    int people = 0;
    int ans = 0;
    for(auto &[persons, ch]: vec) {
        ans += c;
        people += persons;
        int remainingPeople = n - people;
        int remSum = findSum(remainingPeople);
        miniCost = min(remSum + ans, miniCost);
    }
    cout << miniCost;
}
#undef int
