/**
 * Author: omteja04
 * Created on: 25-12-2024 20:11:26
 * Description: Chefland_Library
 **/
...
void levi() {
    IN(int, n);
    vi arr(n);
    std::unordered_map<int, int> returnDays;
    fn(i, n) {
        std::cin >> arr[i];
        int daysLate = i + 1;
        int returnDay = arr[i];
        returnDays[returnDay] = std::max(daysLate, returnDays[returnDay]);
    }
    int total = 0;
    for(auto &entry: returnDays) {
        total += entry.second;
    }
    OUT(total);
}
