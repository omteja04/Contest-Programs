/**
 * Author: omteja04
 * Created on: 16-07-2025 20:12:08
 * Problem: Game_Easy
 * Link: https://www.codechef.com/START195B/problems/GAMEEZ
 **/
#include <bits/stdc++.h>
#include <climits>
#include <utility>
...
void levi() {
    ...
    int maxCoins = 2 * n;
    vector<long long> res(maxCoins + 1, LLONG_MIN);
    ...
    for(int k = 1; k <= maxCoins; ++k) {
        ...
        for(int x = 1; x <= min(n, k); ++x) {
            int twoCoinOps = k - x;
            if(twoCoinOps < 0 || twoCoinOps > x) {
                continue;
            }
            int extraBonus = twoCoinOps * ((x - twoCoinOps) + (x - 1)) / 2;

            long long currentSum = prefix[x] + extraBonus;
