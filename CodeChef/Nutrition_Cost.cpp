/**
 * Author: omteja04
 * Created on: 04-06-2025 20:02:57
 * Problem: Nutrition_Cost
 * Link: https://www.codechef.com/START189B/problems/NUTRICOST
 **/
#include <algorithm>
#include <bits/stdc++.h>
#include <climits>
...
void levi() {
    ...
    INPUT_ARRAY_1D(n, a);
    INPUT_ARRAY_1D(n, b);
    vi minCosts(n + 1, LLONG_MAX);
    for(int i = 0; i < n; i++) {
        int vitaminType = a[i];
        minCosts[vitaminType] = min(minCosts[vitaminType], b[i]);
    }
    int result = 0;
    for(int v = 1; v <= n; v++) {
        if(minCosts[v] < LLONG_MAX) {
            int gain = c - minCosts[v];
            if(gain > 0) {
                result += gain;
            }
        }
