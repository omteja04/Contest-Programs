/**
 * Author: omteja04
 * Created on: 19-03-2025 20:18:28
 * Problem: Big_Difference
 * Link: https://www.codechef.com/START178B/problems/BIGDIF
 **/
#include <bits/stdc++.h>
#include <cstdlib>
#include <vector>
...
void levi() {
    ...
    for(int a: {n, n - 1}) {
        int starting = a % 2 == 0 ? 1 : 2;
        for(int b = starting; b <= a - k; b += 2) {
            int g = gcd(a, b);
            int l = lcm(a, b);
            if(abs(g - l) >= 2 * k) {
                cout << a << " " << b;
